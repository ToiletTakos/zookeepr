...
const getAnimals = (formData = {}) => {
  ...
  // if nothing is passed through the formData the request will simpy GET /api/animals

  //handleGetAnimalsSubmit() gathers all form input packages it up as an object and sents it to the getAnimals()
  // from there the object formData is passed through the object.entries() method to create query parameters
  ...
  fetch(queryUrl)
    .then(response => {
      // when fetching we have to make sure true or false
      if (!response.ok) {
    ...
    .then(animalData => {
      ...
      //once everything is complete (info ok check is done and json parse) it is sent to the printResults which generats cards for each animal
